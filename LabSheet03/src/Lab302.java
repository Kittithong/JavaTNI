...
public class Lab302 {
    public static void main(String[] args) {
        ...
        int maleOption = JOptionPane.showConfirmDialog(null, "Are you biological gender is Male?",
                "Gender",JOptionPane.YES_NO_OPTION);

        if (maleOption == JOptionPane.YES_OPTION) {double weight = (height - 100) * 0.9;
            JOptionPane.showMessageDialog(null, "Hello, Mr. " + name +
                    "\nYour weight should be " + String.format("%.2f", weight) + " kg."
            );
        } else {
            int femaleOption = JOptionPane.showConfirmDialog(null, "Are your biological gender is Female?",
                    "Gender", JOptionPane.YES_NO_OPTION);
